...
@Component({
  selector: 'ngbd-modal-addAsStudent',
  templateUrl: './modal-addAsStudent.html',
  styleUrls: ['./modal-addAsStudent.css']
})
export class NgbdModaladdAsStudent implements OnInit {
  faculties: Faculty[];
  ...
  open(content) {
    this.modalService.open(content, {ariaLabelledBy: 'modal-addAsStudent-title'}).result.then((result) => { 
    });
  }
